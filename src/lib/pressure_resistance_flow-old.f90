...
!*evaluate_PRQ:* Solves for pressure and flow in a rigid or compliant tree structure
  subroutine evaluate_prq(mesh_type,vessel_type,grav_dirn,grav_factor,bc_type,inlet_bc,outlet_bc,COPD_grade)
    ...
    type (prq_solvers) :: perf_model_solve
    ...
    integer, intent(in) :: COPD_grade ! remodeling grade for COPD, 0 = healthy, 1-4 = GOLD I-IV respectively, 5 = smokers
...
!! Allocate memory to depvar arrays
    ...
    allocate (perf_model_solve%mesh_from_depvar(0:2,mesh_dof,0:2), STAT = AllocateStatus)
    if (AllocateStatus /= 0) STOP "*** Not enough memory for perf_model_solve%mesh_from_depvar array ***"
    allocate (perf_model_solve%depvar_at_elem(0:2,depvar_types,num_elems), STAT = AllocateStatus)
    if (AllocateStatus /= 0) STOP "*** Not enough memory for depvar_at_elem array ***"
    allocate (perf_model_solve%depvar_at_node(num_nodes,0:2,depvar_types), STAT = AllocateStatus)
    if (AllocateStatus /= 0) STOP "*** Not enough memory for depvar_at_node array ***"
    allocate (perf_model_solve%prq_solution(mesh_dof,2), STAT = AllocateStatus)
    if (AllocateStatus /= 0) STOP "*** Not enough memory for prq_solution array ***"
    perf_model_solve%prq_solution=0.0_dp !initialise
    allocate (perf_model_solve%FIX(mesh_dof), STAT = AllocateStatus)
    if (AllocateStatus /= 0) STOP "*** Not enough memory for FIX array ***"



!! Setting up mappings between nodes, elements and solution depvar
    call calc_depvar_maps(perf_model_solve%mesh_from_depvar,perf_model_solve%depvar_at_elem,&
                depvar_totals,perf_model_solve%depvar_at_node,mesh_dof,num_vars)

!! Define boundary conditions
    !first call to define inlet boundary conditions
    call boundary_conditions(ADD,perf_model_solve%FIX,bc_type,grav_vect,density,inletbc,outletbc,&
      perf_model_solve%depvar_at_node,perf_model_solve%depvar_at_elem,perf_model_solve%prq_solution,&
      mesh_dof,mesh_type)
    ...
    elseif(mesh_type.eq.'full_plus_ladder')then
        ADD=.TRUE.
        call boundary_conditions(ADD,perf_model_solve%FIX,bc_type,grav_vect,density,inletbc,outletbc,&
            perf_model_solve%depvar_at_node,perf_model_solve%depvar_at_elem,&
            perf_model_solve%prq_solution,mesh_dof,mesh_type)
    endif
...
!! Calculate sparsity structure for solution matrices
    ...
    call calc_sparse_size(mesh_dof,perf_model_solve%depvar_at_elem,perf_model_solve%depvar_at_node,&
      perf_model_solve%FIX,NonZeros,MatrixSize)
    allocate (perf_model_solve%SparseCol(NonZeros), STAT = AllocateStatus)!Note we should be able to calculate the nonzeros and matrix size analtyically then we wont need this.
    if (AllocateStatus /= 0) STOP "*** Not enough memory for SparseCol array ***"
    allocate (perf_model_solve%SparseRow(MatrixSize+1), STAT = AllocateStatus)
    if (AllocateStatus /= 0) STOP "*** Not enough memory for SparseRow array ***"
    allocate (perf_model_solve%SparseVal(NonZeros), STAT = AllocateStatus)
    if (AllocateStatus /= 0) STOP "*** Not enough memory for SparseVal array ***"
    allocate (perf_model_solve%RHS(MatrixSize), STAT = AllocateStatus)
    if (AllocateStatus /= 0) STOP "*** Not enough memory for RHS array ***"
    allocate (perf_model_solve%solver_solution(MatrixSize), STAT = AllocateStatus)
    if (AllocateStatus /= 0) STOP "*** Not enough memory for solver_solution array ***"
    allocate (perf_model_solve%update_resistance_entries(num_elems), STAT = AllocateStatus)
    if (AllocateStatus /= 0) STOP "*** Not enough memory for solver_solution array ***"
    perf_model_solve%update_resistance_entries = 0
    !calculate the sparsity structure
    call calc_sparse_1dtree(bc_type,density,perf_model_solve%FIX,grav_vect,mesh_dof,perf_model_solve%depvar_at_elem, &
        perf_model_solve%depvar_at_node,NonZeros,MatrixSize,perf_model_solve%SparseCol,&
        perf_model_solve%SparseRow,perf_model_solve%SparseVal,perf_model_solve%RHS, &
        perf_model_solve%prq_solution,perf_model_solve%update_resistance_entries,update_flow_nzz_row)
!!! --ITERATIVE LOOP--
    MIN_ERR=1.d10
    N_MIN_ERR=0
      ...
      if(KOUNT.eq.1)then!set up boundary conditions
        if(bc_type.eq.'pressure')then
          ...
          call initialise_solution(inletbc,outletbc,(inletbc-outletbc)/total_resistance, &
              mesh_dof,perf_model_solve%prq_solution,perf_model_solve%depvar_at_node,&
              perf_model_solve%depvar_at_elem,perf_model_solve%FIX)
          !move initialisation to solver solution (skipping BCs).
          no=0
          do depvar=1,mesh_dof !loop over mesh dofs
            if(.NOT.perf_model_solve%FIX(depvar))then
              no=no+1
              perf_model_solve%solver_solution(no)=perf_model_solve%prq_solution(depvar,1)
            endif
          enddo !mesh_dof
        ...
        do ne=1,num_elems !update for all ne
          if(perf_model_solve%update_resistance_entries(ne).gt.0)then
            nz=perf_model_solve%update_resistance_entries(ne)
            perf_model_solve%SparseVal(nz)=-elem_field(ne_resist,ne) !Just updating resistance
          endif
        enddo
        if(bc_type.eq.'flow')then !update RHS to account for element resistance
          do ne=1,num_elems
            depvar = perf_model_solve%depvar_at_elem(1,1,ne)
            if(perf_model_solve%FIX(depvar))then
              perf_model_solve%RHS(update_flow_nzz_row) = perf_model_solve%prq_solution(depvar,1)*elem_field(ne_resist,ne)
            endif
          enddo
...
!! ----CALL SOLVER----
      call pmgmres_ilu_cr(MatrixSize, NonZeros, perf_model_solve%SparseRow, perf_model_solve%SparseCol, &
         perf_model_solve%SparseVal, perf_model_solve%solver_solution, perf_model_solve%RHS, 500, 500,1.d-5,1.d-4,SOLVER_FLAG)
       ...
       elseif(SOLVER_FLAG ==2)then
          ...
          deallocate (perf_model_solve%mesh_from_depvar, STAT = AllocateStatus)
          deallocate (perf_model_solve%depvar_at_elem, STAT = AllocateStatus)
          deallocate (perf_model_solve%depvar_at_node, STAT = AllocateStatus)
          deallocate (perf_model_solve%prq_solution, STAT = AllocateStatus)
          deallocate (perf_model_solve%FIX, STAT = AllocateStatus)
          deallocate (perf_model_solve%solver_solution, STAT = AllocateStatus)
          deallocate (perf_model_solve%SparseCol, STAT = AllocateStatus)
          deallocate (perf_model_solve%SparseVal, STAT = AllocateStatus)
          deallocate (perf_model_solve%SparseRow, STAT = AllocateStatus)
          deallocate (perf_model_solve%RHS, STAT = AllocateStatus)
          deallocate (perf_model_solve%update_resistance_entries, STAT=AllocateStatus)
          exit
       endif
!!--TRANSFER SOLVER SOLUTIONS TO FULL SOLUTIONS
      ...
      do depvar=1,mesh_dof
         if(.NOT.perf_model_solve%FIX(depvar)) THEN
            no=no+1
            perf_model_solve%prq_solution(depvar,2)=perf_model_solve%prq_solution(depvar,1) !temp storage of previous solution
            perf_model_solve%prq_solution(depvar,1)=perf_model_solve%solver_solution(no) !new pressure & flow solutions
            if(DABS(perf_model_solve%prq_solution(depvar,1)).GT.0.d-6)THEN
               ERR=ERR+(perf_model_solve%prq_solution(depvar,2)-&
                perf_model_solve%prq_solution(depvar,1))**2.d0/perf_model_solve%prq_solution(depvar,1)**2
            endif
         endif
      enddo !no2
...
!Update vessel radii based on predicted pressures and then update resistance through tree
        call calc_press_area(grav_vect,KOUNT,perf_model_solve%depvar_at_node,perf_model_solve%prq_solution,&
         ...
         if(mesh_type.eq.'full_plus_ladder')then
           do ne=1,num_elems
              if(elem_field(ne_group,ne).eq.1.0_dp)then!(elem_field(ne_group,ne)-1.0_dp).lt.TOLERANCE)then
                ...
                P1=perf_model_solve%prq_solution(perf_model_solve%depvar_at_node(elem_nodes(2,ne0),0,1),1) !pressure at start node of capillary element
                P2=perf_model_solve%prq_solution(perf_model_solve%depvar_at_node(elem_nodes(1,ne1),0,1),1)!pressure at end node of capillary element
                Q01=perf_model_solve%prq_solution(perf_model_solve%depvar_at_elem(1,1,ne0),1) !flow in element upstream of capillary element !mm^3/s
...
!need to write solution to element/nodal fields for export
    call map_solution_to_mesh(perf_model_solve%prq_solution,perf_model_solve%depvar_at_elem,&
                               perf_model_solve%depvar_at_node,mesh_dof)
...
!local variables
    ...
    real(dp) :: lum_hyp,nfactor,counter2,prune_fraction 
    real(dp) :: remodel_fraction,counter1
    integer :: rad_pointer 
    ...
    !!! EDIT THESE FRACTIONS TO HAVE MUCH MORE REMODELLING THAN PRUNING????????
    !!! Notes 28th May meeting with Behdad + Kelly: turn pruning fraction into emphysema related injury??
    if(grade .eq. 0) then ! Normal non-smokers
       prune_fraction = 0.0_dp
       remodel_fraction = 0.0_dp     
    elseif(grade .eq. 1) then   ! GOLD I 
       prune_fraction = 0.05_dp
       remodel_fraction = 0.1_dp   
    elseif(grade .eq. 2) then       ! GOLD II
       prune_fraction = 0.125_dp
       remodel_fraction = 0.25_dp   
    elseif(grade .eq. 3) then   ! GOLD III    
       prune_fraction = 0.275_dp
       remodel_fraction = 0.55_dp   
    elseif(grade .eq. 4) then   ! GOLD IV    
       prune_fraction = 0.35_dp
       remodel_fraction = 0.7_dp                                           
    elseif(grade .eq. 5) then   ! Smokers    
       prune_fraction = 0.035_dp
       remodel_fraction = 0.07_dp 
    elseif(grade .eq. 21) then ! Sensitivity 1 == remodel +
       prune_fraction = 0.125_dp
       remodel_fraction = 0.25_dp + 0.10_dp     
    elseif(grade .eq. 22) then ! Sensitivity 2 == remodel -
       prune_fraction = 0.125_dp
       remodel_fraction = 0.25_dp - 0.10_dp
    elseif(grade .eq. 23) then ! Sensitivity 3 == prune +
       prune_fraction = 0.125_dp + 0.10_dp
       remodel_fraction = 0.25_dp 
    elseif(grade .eq. 24) then ! Sensitivity 4 == prune -
       prune_fraction = 0.125_dp - 0.10_dp
       remodel_fraction = 0.25_dp  
    elseif(grade .eq. 25) then ! Sensitivity 5 == both +
       prune_fraction = 0.125_dp + 0.05_dp
       remodel_fraction = 0.25_dp + 0.05_dp 
    elseif(grade .eq. 26) then ! Sensitivity 6 == both -
       prune_fraction = 0.125_dp - 0.05_dp
       remodel_fraction = 0.25_dp - 0.05_dp   
    endif       
    counter1 = 1.0_dp
    ...
    do ne=1,num_elems
      do nn=1,2
        ...
        if(nn.eq.1) then 
          ...
          rad_pointer = ne_radius_in
        elseif(nn.eq.2) then
          R0=elem_field(ne_radius_out0,ne)
          rad_pointer = ne_radius_out
        endif
        lum_hyp = 1.0_dp
        nfactor = 1.0_dp        
        if(node_field(nj_gtv,np).eq.1) then ! applying tumour volume 
           nfactor =  0.15_dp ! impossible to see the effects on CT for narrowing
           lum_hyp = 1.0_dp/6.0_dp! Tumour stiffness goes up by 6
        elseif(node_field(nj_emph,np).le.-950_dp) then ! Applying emphysema to vessels
           if (elem_field(ne_group,ne).eq.0.0_dp.and.elem_ordrs(no_sord,ne).eq.1) then
              nfactor = 0.005_dp  ! pruning of peripheral arterial vessels
           else
              nfactor = 1.0_dp ! no effect on narrowing for vessels upstream
              Ptm = 3.0_dp*Ptm ! Local increase in Ptm
              lum_hyp = 1.0_dp/100_dp
              !1.3_dp-0.0001_dp*node_field(nj_emph,np) !Local compliance increase ! https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2692121/
           endif
        elseif(elem_field(ne_group,ne).eq.0.0_dp) then !only applying on arteries         
         if(elem_ordrs(no_sord,ne).eq.1) then !elem_ordrs(no_sord,ne).eq.1)
           if(counter2/100.le.prune_fraction) then ! pruning a fraction of the terminal arteries
      ...
      if(vessel_type.eq.'elastic_g0_beta')then 
        if(Ptm.lt.elasticity_parameters(3).and.elasticity_parameters(1).gt.0.0_dp) then
          elem_field(rad_pointer,ne)=R0*((Ptm/elasticity_parameters(1)) &
             +1.d0)**(1.d0/elasticity_parameters(2))
        elseif(Ptm.lt.0.0_dp.or.elasticity_parameters(1).LT.TOLERANCE)then
          ...
          elem_field(rad_pointer,ne)=R0
        else!ptm>ptmmax
          elem_field(rad_pointer,ne)=R0*((elasticity_parameters(3)/elasticity_parameters(1)) & 
               +1.d0)**(1.d0/elasticity_parameters(2))             
        endif
      elseif(vessel_type.eq.'elastic_alpha')then
        if(Ptm.LT.elasticity_parameters(2))then
          elem_field(rad_pointer,ne)=nfactor*R0*((lum_hyp*Ptm*elasticity_parameters(1))+1.d0)             
        elseif(Ptm.lt. 0.0_dp)then
          if(Ptm.lt.0)write(*,*) 'Transmural pressure < zero',ne,Ptm,Pblood,Ppl
          elem_field(rad_pointer,ne)=nfactor*R0
        else!ptm>ptmmax
          elem_field(rad_pointer,ne)=nfactor*R0*((lum_hyp*elasticity_parameters(2)*elasticity_parameters(1)) &
               +1.d0)
        endif
      elseif(vessel_type.eq.'elastic_hooke')then
        h=elasticity_parameters(2)*R0
        elem_field(rad_pointer,ne)=R0+3.0_dp*R0**2*Ptm/(4.0_dp*elasticity_parameters(1)*h)           
      else
...
!####################################
!
subroutine set_factors(test_grade)

    integer, intent(in) :: test_grade
